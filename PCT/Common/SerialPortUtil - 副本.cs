...
namespace PCT.Common
{
    ...
    public class SerialPortUtil
    {
        ...
        public List<byte> GetPortData333()
        {
            int n = comPort.BytesToRead;//先记录下来，避免某种原因，人为的原因，操作几次之间时间长，缓存不一致
            byte[] buf = new byte[n];//声明一个临时数组存储当前来的串口数据
            int count = comPort.Read(buf, 0, n);//读取缓冲数据
            System.IO.StreamWriter sw = new System.IO.StreamWriter("d:\\sc33.txt", true);
            sw.WriteLine(string.Format("{0}\t{1}", System.DateTime.Now.ToString(), SerialPortUtil.ByteToHex(buf)));
            sw.Close();
        ...
        public List<byte> GetPortData()
        {
            ...
            while (comPort.BytesToRead > 0 || !found || _byteData.Count<11)
            {
